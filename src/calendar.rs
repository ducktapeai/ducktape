...
fn create_single_event(config: EventConfig) -> Result<()> {
    ...
    let recurrence_block = if let Some(recurrence) = &config.recurrence {
        ...
        debug!("Generated recurrence rule: {}", rrule);
        
        format!(r#"
                -- Set recurrence using RFC 5545 format
                tell newEvent
                    set recurrence to "{}"
                end tell"#, 
            rrule
