            ...
            match cmd {
                Commands::Calendar { action } => {
                    ...
                    match action {
                        ...
                        CalendarActions::Create { title, date, start_time, end_time, calendar, contacts, email, location, notes, zoom, repeat, interval, until, count, days } => {
                            ...
                            if let Some(d) = days {
                                let days_str = d.iter().map(|&day| day.to_string()).collect::<Vec<String>>().join(",");
