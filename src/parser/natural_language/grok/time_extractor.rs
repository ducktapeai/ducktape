...
pub fn extract_time_with_timezone(input: &str) -> Option<(DateTime<Tz>, Option<Tz>)> {
    ...
    if let Some(caps) = TIME_WITH_ZONE_RE.captures(input) {
        ...
        for i in 0..=4 {
            // Iterate up to the number of groups expected
            println!(
        ...
        println!(
            ...
            day_specifier_match, time_str_from_group, tz_abbr_str
        );

        if time_str_from_group.is_empty() || tz_abbr_str.is_empty() {
            println!(
                "DEBUG: extract_time_with_timezone: Empty time_str or tz_abbr after capture. Returning None."
            );
            return None;
        }
        ...
        if let Some(tz) = map_abbreviation_to_iana(&tz_abbr_str) {
            ...
            if let Some(naive_dt) = parse_time_with_possible_day(
                time_str_from_group,
                day_specifier_match.as_deref(),
                Some(tz),
            ) {
...
fn parse_time_with_possible_day(
    ...
    if am_pm_present {
        debug!(
            "parse_time_with_possible_day: Using FLEXIBLE_TIME_AMPM_RE: {}",
            FLEXIBLE_TIME_AMPM_RE.as_str()
        ); // Added this line
