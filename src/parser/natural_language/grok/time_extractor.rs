    ...
    for (pattern, date_text) in &time_patterns {
        ...
        if let Some(time_caps) = time_re.captures(title) {
            ...
            let cmd_suffix = if let Some(pos) = command.find(" \"Calendar\"") {
                ...
                if pos_after_calendar < command.len() {
                    &command[pos_after_calendar..]
                } else {
                    ""
                }
            } else {
...
mod tests {
    ...
    fn test_extract_time_from_title() {
        ...
        let command = r#"ducktape calendar create "test tonight at 7pm" today 00:00 01:00 "Calendar""#;
        ...
        let command = r#"ducktape calendar create "Meeting tomorrow at 9am" today 00:00 01:00 "Calendar""#;
        let fixed = extract_time_from_title(command, "");
        ...
        let command = r#"ducktape calendar create "Call today at 3:30pm" today 00:00 01:00 "Calendar""#;
        let fixed = extract_time_from_title(command, "");
