...
pub fn extract_time_with_timezone(input: &str) -> Option<(DateTime<Tz>, Option<Tz>)> {
    ...
    if let Some(caps) = TIME_WITH_ZONE_RE.captures(input) {
        ...
        println!("DEBUG: extract_time_with_timezone: Assigned after capture: Day spec: {:?}, Time str: '{}', TZ abbr: '{}'", day_specifier_match, time_str, tz_abbr);


        if let Some(tz) = map_abbreviation_to_iana(&tz_abbr) {
            println!("DEBUG: extract_time_with_timezone: Mapped abbr '{}' to IANA: {}", tz_abbr, tz.name());
            if let Some(naive_dt) = parse_time_with_possible_day(time_str, day_specifier_match, Some(tz)) {
                ...
                println!("DEBUG: extract_time_with_timezone: parse_time_with_possible_day failed for time_str '{}', day_specifier_match '{:?}' with tz {}", time_str, day_specifier_match, tz.name());
                return None;
            }
        } else {
            println!("DEBUG: extract_time_with_timezone: map_abbreviation_to_iana failed for abbr '{}'", tz_abbr);
        }
    } else {
            ...
            if let Some(ampm_str) = ampm_str_opt {
                 if let (Ok(h_val), Ok(m_val)) = (hour_str.parse::<u32>(), minute_str.parse::<u32>()) {
                    ...
                    if h_val >= 1 && h_val <= 12 && m_val < 60 {
                        ...
                        println!("DEBUG: parse_time_with_possible_day: Parsed AM/PM time: h={}, m={}, ampm={}", h_val, m_val, ampm_str_clone);
                    } else {
